...
public class Item
{
    ...
    public virtual void SetView()
    {
        ...
        if (!string.IsNullOrEmpty(prefabname))
        {
            ...
            //if (prefab)
            //{
                TilePool.Instance.ActivePoolObject(prefabname,Vector3.zero);
             
                View = TilePool.Instance.poolObject.transform;
                if (prefabname != Constants.PREFAB_BONUS_BOMB && prefabname != Constants.PREFAB_BONUS_VERTICAL && prefabname != Constants.PREFAB_BONUS_HORIZONTAL)
                {
                    ReSkin.Instance.StartReSkin(View.gameObject, (int)GetTypeItem());
    ...
    public virtual eNormalType GetTypeItem() { return eNormalType.TYPE_ONE; }
        ...
        if (View)
        {
            View.DOScale(0.1f, 0.1f).OnComplete(
                () =>
                {
                    ...
                    View.transform.SetParent(TilePool.Instance.GetScripPool(GetPrefabName()).transform);
                    View = null;
                }
                );
        }
